...
def load_data():
    ...
    try:
        ...
        # Try to parse sentiment date (might be YYYY-MM-DD or DD-MM-YYYY)
        try:
        ...
        # Convert trade timestamp - handle DD-MM-YYYY HH:MM format
        try:
            df_trades['Timestamp IST'] = pd.to_datetime(df_trades['Timestamp IST'], format='%d-%m-%Y %H:%M')
        except:
            try:
                df_trades['Timestamp IST'] = pd.to_datetime(df_trades['Timestamp IST'], format='%Y-%m-%d %H:%M:%S')
            except:
                # Let pandas infer the format with dayfirst=True for DD-MM-YYYY
        ...
        st.info("Date formats supported: YYYY-MM-DD, DD-MM-YYYY, DD-MM-YYYY HH:MM")
